# Romaric Vossanou

**Backend Developer** | Python & Django Enthusiast | Mathematics & Computer Science Student

---

## 👋 About Me

I'm a third-year Mathematics and Computer Science student at the University of Abomey-Calavi, specializing in backend development. I'm passionate about building robust APIs, database management, and clean architecture.

- 🎓 **Education:** Licence 3 in Mathematics, Computer Science and Applications
- 📍 **Location:** Cotonou, Benin
- 💼 **Availability:** August 2025 - January 2026 (6-month internship)
- 🌱 **Currently Learning:** Advanced Django, API design, and database optimization
- 🎯 **Goal:** Join an innovative startup to grow my backend development skills

## 🛠️ Technical Skills

**Backend Development:**
- Python, Django, Django REST Framework
- API Design & Development
- Database Management (PostgreSQL, SQLite)
- Authentication & Authorization

**Tools & Technologies:**
- Git & GitHub
- HTML, CSS, JavaScript (Frontend basics)
- RESTful API principles
- Problem-solving & Algorithm design

## 🚀 Projects

### [Ecom](https://github.com/RYV8/Ecom)
E-commerce backend built with Python and Django, featuring product management and API endpoints.

### [Projet Chat API](https://github.com/RYV8/Projet_Chat_API)
Real-time chat API project demonstrating backend communication principles.

### [Portfolio](https://github.com/RYV8/portofolio1)
Personal portfolio website showcasing my work and skills (HTML, CSS, JavaScript).

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=RYV8&show_icons=true&theme=default&hide_border=true)

## 📫 Get in Touch

I'm always open to discussing new opportunities, collaborations, or just chatting about technology!

- 💼 Open to internship opportunities in Cotonou
- 🤝 Looking to collaborate on Python/Django projects
- 📧 Feel free to reach out through GitHub

---

*"Learning every day, building for tomorrow"*
